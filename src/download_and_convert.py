...
def process_month(year, month, base_url, project_root, keep_intermediates=False):
    ...
    parquet_path = project_root / 'data' / 'parquet' / 'trips' / f"year={year}" / f"month={month:02d}" / 'trips.parquet'
    ...
    # Step 1: Download
    print(f"[1/4] Downloading from S3...")
    ...
    print(f"[3/4] Converting to Parquet...")
    ...
    print(f"  ✓ CSV: {conversion_stats['csv_size_mb']:.1f} MB → Parquet: {conversion_stats['parquet_size_mb']:.1f} MB")
    ...
    if not keep_intermediates:
        ...
        try:
            ...
            try:
                ...
                pass
            print(f"  ✓ Removed intermediate files")
        except Exception as e:
            print(f"  Warning: Could not remove files: {e}")
    else:
        print(f"[4/4] Keeping intermediate files")
...
def main(years=[2024, 2025], keep_intermediates=False):
    ...
    if successful:
        ...
        print(f"Total Parquet size: {total_parquet_size:.1f} MB ({total_parquet_size/1024:.2f} GB)")
