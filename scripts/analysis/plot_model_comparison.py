...
bars1 = ax1.bar(
    x - width / 2, temporal_vals, width, label="Temporal Flow", color="#2ecc71", alpha=0.8
)
...
ax2.set_title(
    "State Detection Performance\n(Higher is Better)", fontsize=12, fontweight="bold", pad=15
)
ax2.set_xticks(x)
...
ax3.plot(
    fold_ids, markov_mae, marker="s", linewidth=2, markersize=6, label="Markov", color="#e74c3c"
)
...
ax3.set_title(
    "MAE Over Time\n(Both Models Improve with More Data)", fontsize=12, fontweight="bold", pad=15
)
...
ax4.plot(
    fold_ids, markov_empty, marker="s", linewidth=2, markersize=6, label="Markov", color="#e74c3c"
)
...
ax5.plot(
    fold_ids, markov_full, marker="s", linewidth=2, markersize=6, label="Markov", color="#e74c3c"
)
...
ax.set_title(
    "Temporal Flow MAE Advantage by Fold\n(Positive = Temporal Better)",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
ax.axhline(
    y=mean_diff, color="blue", linewidth=2, linestyle="--", label=f"Mean: {mean_diff:+.2f} bikes"
)
