...
def forward_pass_fast(trips: pd.DataFrame, stations: list) -> dict:
    ...
    valid_trips = trips[
        trips["start_station_name"].isin(stations) & trips["end_station_name"].isin(stations)
    ].copy()
...
def process_month(month_dir: Path, stations: list) -> dict:
    ...
    month_stations = set(trips["start_station_name"].dropna()) | set(
        trips["end_station_name"].dropna()
    )
...
def main():
    ...
    for month_dir in month_dirs:
        ...
        if all_stations is None:
            trips = load_trips_for_month(month_dir)
            all_stations = list(
                set(trips["start_station_name"].dropna()) | set(trips["end_station_name"].dropna())
            )
