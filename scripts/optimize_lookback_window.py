...
def run_experiment(config, train_weeks, test_weeks, increment_days=7):
    ...
    fold_results, summary = run_cross_validation(
        model, trips, station_stats, config, verbose=False
    )
    ...
    for train_w, test_w, inc_days, label in configurations:
        try:
            fold_results, summary = run_experiment(
                config.copy(), train_w, test_w, inc_days
            )
...
def create_plots(results, output_dir):
    ...
    metrics_data = {}
    for metric in ["inventory_mae", "inventory_rmse", "empty_recall", "full_recall", "state_accuracy"]:
    ...
    fig.suptitle('Markov Model: Lookback Window Optimization\n(Error bars show ± 1 std across folds)',
    ...
    ax.set_title('Full Station Detection\n(Higher is Better)', fontsize=13, fontweight='bold', pad=10)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=9)
    ax.grid(axis='y', alpha=0.3)
    ax.set_ylim([0, 100])
    ...
    ax.set_title('Overall State Classification\n(Higher is Better)', fontsize=13, fontweight='bold', pad=10)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=9)
    ax.grid(axis='y', alpha=0.3)
    ax.set_ylim([0, 100])
    ...
    scatter = ax.scatter(n_folds, mae_means, s=200, c=range(len(labels)),
                        cmap='viridis', alpha=0.7, edgecolors='black', linewidths=2)
    ax.errorbar(n_folds, mae_means, yerr=mae_stds, fmt='none', ecolor='gray',
                capsize=5, alpha=0.5, linewidth=2)

    # Annotate points
    for i, (nf, mae, label) in enumerate(zip(n_folds, mae_means, labels)):
        ax.annotate(label.split(',')[0], (nf, mae), fontsize=8,
                   xytext=(5, 5), textcoords='offset points')

    ax.set_xlabel('Number of Folds', fontsize=12, fontweight='bold')
    ax.set_ylabel('MAE (bikes)', fontsize=12, fontweight='bold')
    ax.set_title('Computation vs Accuracy Trade-off\n(More folds = more compute)',
    ...
    for metric, (means, stds) in metrics_data.items():
        if 'mae' in metric or 'rmse' in metric:
            ...
            normalized = [(max(means) - m) / (max(means) - min(means)) if max(means) != min(means) else 0.5
                         for m in means]
        else:
            # Higher is better
            normalized = [(m - min(means)) / (max(means) - min(means)) if max(means) != min(means) else 0.5
                         for m in means]
    ...
    for i, (result, color) in enumerate(zip(results, colors)):
        ...
        ax.bar(offsets, values, width, yerr=errors, capsize=3, label=result["label"], alpha=0.8,
               color=color, edgecolor='black', linewidth=0.5)

    ax.set_ylabel('Normalized Performance (0=worst, 1=best)', fontsize=12, fontweight='bold')
    ax.set_title('Lookback Window Configuration Comparison\n(All metrics normalized for comparison)',
...
def print_recommendations(results):
    ...
    if short_train and long_train:
        ...
        if short_mae < long_mae:
            print(f"  • Shorter training windows perform better (avg MAE: {short_mae:.2f} vs {long_mae:.2f})")
        else:
            print(f"  • Longer training windows perform better (avg MAE: {long_mae:.2f} vs {short_mae:.2f})")
